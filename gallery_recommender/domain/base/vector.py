...
from gallery_recommender.application.networks.embeddings import EmbeddingModelSingleton
...
from gallery_recommender.infrastructure.db.qdrant import connection
from gallery_recommender.infrastructure.db.qdrant import QdrantDatabaseConnector

T = TypeVar("T", bound="VectorBaseData")


class VectorBaseData(BaseModel, Generic[T], ABC):
    ...
    @classmethod
    def bulk_insert(cls: Type[T], documents: list["VectorBaseData"]) -> bool:
        try:
    ...
    @classmethod
    def _bulk_insert(cls: Type[T], documents: list["VectorBaseData"]) -> None:
    ...
    def _search(cls: Type[T], query_vector: list, limit: int = 10, **kwargs) -> list[T]:
        ...
        logger.info(f"Qdrant connection info at search: {QdrantDatabaseConnector.get_connection_info()}")
        try:
            ...
            **kwargs,
            ...
            logger.error(f"Failed to search documents in '{cls.get_collection_name()}': {e}")
            documents = []
    ...
    @classmethod
    def create_payload_schema(cls: Type[T]) -> bool:
        collection_name = cls.get_collection_name()
        field_name = cls.get_field_name()
        return cls._create_payload_schema(collection_name=collection_name, field_name=field_name)


    @classmethod
    def _create_payload_schema(cls: Type[T], collection_name: str, field_name: str) -> PayloadSchemaType:
        return connection.create_payload_index(
            collection_name=collection_name, 
    ...
    @classmethod
    def delete_qdrant(cls: Type[T], collection_name: str) -> bool:
        return connection.delete_collection(collection_name=collection_name)
    ...
    @classmethod
    def group_by_class(
        cls: Type["VectorBaseData"], documents: list["VectorBaseData"]
    ) -> Dict["VectorBaseData", list["VectorBaseData"]]:
    ...
    @classmethod
    def collection_name_to_class(cls: Type["VectorBaseData"], collection_name: str) -> type["VectorBaseData"]:
